...
export default function withAdminAuth(Component) {
  return function AdminAuthenticatedComponent(props) {
    ...
    useEffect(() => {
      if (user.isHydrated) {
        if (!user.isAuthenticated || !(user.role?.includes("superadmin") || user.role?.includes("admin"))) {
          setRedirecting(true);
  ...
  if (user.isHydrated && user.isAuthenticated && (user.role?.includes("superadmin") || user.role?.includes("admin"))) {
